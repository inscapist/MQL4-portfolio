...
int start() {  	
   ...
   BoxPoints = multiplier*iATR(NULL,0,atr_period,0);
...
// Init
	   ...
	   historyloader();
		...
		//---------------------------------------------------------------------------
         ...
         BoxPoints = multiplier*iATR(NULL,0,atr_period,i);
		...
		Comment("RenkoLiveChart(" + atr_period + "): Open Offline ", SymbolName, ",M", ATRRenkoTimeFrame, " to view chart");
...
// Error checking	
 ...
 int renkoInit() {
		...
		BoxPoints = multiplier*iATR(NULL,0,atr_period,Bars-15);
